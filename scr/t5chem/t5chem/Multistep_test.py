...
import torch
import rdkit
from t5chem import EarlyStopTrainer, SimpleTokenizer
...
class T5ForMultiStep(T5ForMultiStepBase):
    ...
    def generate(self, input_ids,attention_mask, **kwargs):       # get output tensor for a batch
        with torch.no_grad():
            this_input = input_ids[0]
            pred_reac_interm_dict = {}  
        if len(input_ids) == 1:        # if the whole batch are all single step reaction.
            attention_mask = attention_mask[0]
        else:
            for i in range(len(input_ids)-1):
                pred_reac_interm_list, inputs = generate_step(self.generator, this_input, input_ids[i+1])
                pred_reac_interm_dict[i] = pred_reac_interm_list        # collect batched intermediates for the ith+1 reaction step.
                this_input = inputs['input_ids'].to(device)
        ...
        return outputs, pred_reac_interm_dict
...
def generate_step(model, cur_inputs, next_input):
    ...
    outputs = model.generate(calcul_cur_inputs, num_beams=5, max_length=150, early_stopping=True)   # This .generate take us to method GenerationMixin.generate of T5ForConditionalGeneration()
    next_inputs = []
    cur_step_interms = []
    for i in range(batch_size):
        if i in calcul_index_list:            
            ...
            cur_step_interms.append(pred_str)       
        elif i in skip_index_list:
            next_str = tokenizer.decode(next_input[i], skip_special_tokens=True)
            next_inputs.append( next_str ) 
            cur_step_interms.append("")
        else:
            print("------The index for def generate_step dosen't match: ", i)
    return cur_step_interms, tokenizer(next_inputs, padding="longest", return_tensors='pt')
if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_dir", type=str,
                        default="./0_shots/",
                        help="Directory that contains test.source/test.target etc.")
    parser.add_argument("--model_dir", type=str,
                        default="./Original_T5chem/",
                        help="Directory of the pretrained model/checkpoint.")
    parser.add_argument("--output_name", type=str,
                        default="predictions.csv",
                        help="Output CSV filename (saved under model_dir unless you give a full path).")
    ...
    targets = []
    with open(os.path.join( data_dir , "test.target")) as rf:
        for line in rf:
            targets.append(standize(line.strip()[:150]))
    ...
    for batch in tqdm(test_loader, desc="prediction"):
        for k, v in batch.items():
            if isinstance(v, torch.Tensor):
                ...
                batch[k] = [vs.to(device) for vs in v]
        del batch['labels']
        with torch.no_grad():
            outputs, intermedias_dict = model.generate(**batch, **task_specific_params['Reaction'])

        if intermedias_dict :
        # collect intermedias for samples in a batch
            batch_size = len(intermedias_dict[0])
            num_reaction_step = len(intermedias_dict)
            for index_sample in range(batch_size):   
                interms_current = [ intermedias_dict[i][index_sample] for i in range(num_reaction_step) if intermedias_dict[i][index_sample] != ""] 
                if interms_current == []:
                    print("----------- There's no predicted intermediats for current sample ---------- ")
                    print("----------- Please check if there's single step reaction in test set ---------- ")
                interms_current = {index: value for index, value in enumerate(interms_current)}
                intermedias_All.append(interms_current)
    ...
    test_df['rank'] = test_df.apply(lambda row: get_rank(row, 'prediction_', 5), axis=1)
    
    if intermedias_All:
        max_reaction_steps = max(len(d.keys()) for d in intermedias_All)
        for step in range(max_reaction_steps):
            test_df[f'predicted_step{step+1}_intermediates'] = [dict[step]  if step in dict else '' for dict in intermedias_All]
    ...
    # output file dir: 如果 output_name 是绝对路径/包含目录，就直接用；否则默认放在 model_dir 下
