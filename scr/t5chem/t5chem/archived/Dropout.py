...
#class EarlyStopTrainerWithDropout(Trainer):
...
class T5ForPropertyWithDropout(nn.Module):
    def __init__(self, pretrain_path, seq2seqW=1):
        super().__init__()
        self.RegressionModel = T5ForProperty.from_pretrained(pretrain_path, head_type='regression')
        print(self.RegressionModel)
        self.RegressionModel.lm_head[0]
        #self.lm_head_layers[]
...
if __name__ == "__main__":
    ...
    model = T5ForPropertyWithDropout("/scratch/yw5806/USPTO_500_MT")
    tokenizer = SimpleTokenizer(vocab_file="/scratch/yw5806/USPTO_500_MT/vocab.pt")
    os.makedirs("./try_dropout/", exist_ok=True)
    tokenizer.save_vocabulary(os.path.join("./try_dropout/", 'vocab.pt'))

    import sys
    sys.path.append('/scratch/yw5806/environments/t5chem/t5chem')
    from data_utils import (AccuracyMetrics, CalMSELoss, LineByLineTextDataset,
                        T5ChemTasks, TaskPrefixDataset, TaskSettings,
                        data_collator)
    
    data_dir = "/scratch/yw5806/noisy_lables/noisy_data/HTP_P/yield/"
    dataset = TaskPrefixDataset(
            ...
            max_target_length=1,
            separate_vocab=("regression" != 'seq2seq'),
            type_path="train",
        )
    ...
    trainer = EarlyStopTrainerWithDropout(
        model=model,
        args=training_args,
