...
class TaskPrefixDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self.sep_vocab:
            ...
            target_ids = target_sample["input_ids"].squeeze(0).to(torch.long)
            # target_ids = target_sample["input_ids"].squeeze(0) ????????????? maybe we need the tensor in the other steps
...
class MultiStepForwardDataset(Dataset):
    def __init__(
        self,
        tokenizer: PreTrainedTokenizer,
        data_dir: str,
    ...
    def __getitem__(self, idx: int) -> Dict[str, List[int]]:
        source_line: str = linecache.getline(self._source_path, idx + 1).strip()
        source_items: List[str] = source_line.split('//')
        source_sample: BatchEncoding = self.tokenizer(
                        ["Product:"+x for x in source_items],
                        max_length=self.max_source_len,
                        padding='do_not_pad',
                        truncation=True,
                    )
        ...
        target_ids = target_sample["input_ids"]
        source_ids: torch.Tensor = source_sample["input_ids"]
        src_mask: torch.Tensor = source_sample["attention_mask"]
        return {"input_ids": source_ids, "attention_mask": src_mask,
                "decoder_input_ids": target_ids}
