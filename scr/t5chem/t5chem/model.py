...
class T5ForMultiStepBase(T5PreTrainedModel):
    def __init__(
        self,
        config: T5Config,
        ) -> None:
        super().__init__(config)
        self.generator = T5ForConditionalGeneration(config)
        self.model_dim = config.d_model

        self.model_parallel = False
        self.device_map = None

    def estimate_tokens(self, input_dict) -> int:
        token_inputs = [tensor for key, tensor in input_dict.items() if "input" in key]
        if token_inputs:
            return sum([torch.cat(token_input, dim=1).numel() for token_input in token_inputs])
        else:
            warnings.warn(
